...
fun shamirCipherLong(dataSource: Pair<String, String>, dump: Boolean = true) {
    ...
    val file = "$resource/${dataSource.first}${dataSource.second}".fileToLongList()
    ...
    encrypt.writeTo("$path/encrypt/", "${dataSource.first}_encrypt${dataSource.second}")
    val decrypt = wrapper.decrypt(encrypt, true)
    decrypt.writeTo("$path/decrypt/", "${dataSource.first}_decrypt${dataSource.second}")
}
...
fun rsaCipherLongSignature(dataSource: Pair<String, String>, dump: Boolean = true) {
    ...
    val file = "$resource/${dataSource.first}${dataSource.second}".fileToByteArray()
    ...
    verify.writeTo("$path/verify/", "${dataSource.first}_verify.txt")
}
